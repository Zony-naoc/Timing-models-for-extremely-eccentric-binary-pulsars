...
void checkLine(pulsar *psr,char *str,FILE *fin,parameter *elong, parameter *elat)
{
    ...
    /*  else if (strcasecmp(str,"OMDOT")!=0 && (str[0]=='O' || str[0]=='o') &&
        ...
        else if (strcasecmp(str,"ZETA1")==0)
            readValue(psr,str,fin,&(psr->param[param_zeta1]),0); 
        else if (strcasecmp(str,"ZETA2")==0)
            readValue(psr,str,fin,&(psr->param[param_zeta2]),0);
        else if (strcasecmp(str,"ZETA3")==0)
            readValue(psr,str,fin,&(psr->param[param_zeta3]),0);
...
void readParfile(pulsar *psr,char parFile[][MAX_FILELEN],char timFile[][MAX_FILELEN],int npsr)
{
    ...
    parameter elong,elat;
    int noread=0,endit;
    const char *CVS_verNum = "$Id: 7865f25374ca90a39eefc7044782cb4ec1eed5b2 $";
