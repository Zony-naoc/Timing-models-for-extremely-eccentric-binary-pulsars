...
void formResiduals(pulsar *psr,int npsr,int removeMean)
{
    ...
    const char *CVS_verNum = "$Id: b2db0ca8865a149f7fd9e312b83a6a53958836ba $";
    ...
    for (p=0;p<npsr;p++)
    {
        ...
        for (i=0;i<psr[p].nobs;i++)
        {
            ...
            if (psr[p].param[param_pb].paramSet[0]==1 || psr[p].param[param_fb].paramSet[0]==1) 
            {
                ...
                else if (strcmp(psr[p].binaryModel,"DDE")==0)   torb = DDEmodel(psr,p,i,-1);
                else if (strcmp(psr[p].binaryModel,"DDEGR1")==0)  torb = DDEGR1model(psr,p,i,-1);
		        ...
		        else if (strcmp( psr[p].binaryModel, "ELL1k" ) ==0)  torb = ELL1kmodel( psr, p, i, -1, 0);
