...
double t2FitFunc_binaryModels(pulsar *psr, int ipsr ,double x ,int ipos ,param_label label,int k){
    ...
    if (strcmp(psr->binaryModel,"BT")==0) /* Must be below other parameters */
    ...
    else if (strcmp(psr->binaryModel,"DDE")==0)
        afunc = DDEmodel(psr,0,ipos,label);
    else if (strcmp(psr->binaryModel,"DDEGR1")==0)
        afunc = DDEGR1model(psr,0,ipos,label);
        ...
        afunc = ELL1kmodel(psr,0,ipos,label,k);
...
void t2UpdateFunc_binaryModels(pulsar *psr, int ipsr ,param_label label,int k, double val, double error){
    if (strcmp(psr[ipsr].binaryModel,"BT")==0)
    ...
    else if (strcmp(psr[ipsr].binaryModel,"DDE")==0)
        updateDDE(&psr[ipsr],val,error,label);
    else if (strcmp(psr[ipsr].binaryModel,"DDEGR1")==0)
        updateDDEGR1(&psr[ipsr],val,error,label);
        ...
        updateELL1k(&psr[ipsr],val,error,label,k);
}
