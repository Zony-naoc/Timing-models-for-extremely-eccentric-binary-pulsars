...
void mass2ddegr(double am,double am2,double x,double ecc,double an,double *arr,double *ar,
        double *xk,double *si,double *gamma,double *pbdot);


longdouble DDEGR1model(pulsar *psr,int p,int ipos,int param) //DDEGR+ model
{
    double an,afac;
    double pb,k;
    ...
    double pbdot,xpbdot,phase,gamma,m,m1,arr,ar,xk,xomdot;
    double fac,du,u;
    ...
    double cSINI; //If use sini as input

    longdouble czeta1,czeta2,czeta3;
    longdouble zeta1,zeta2,zeta3;
    double xkex;

    const char *CVS_verNum = "$Id: 5173aaeb0c12fdcb2808e5e76aec047a0f0043f5 $";

    if (displayCVSversion == 1) CVSdisplayVersion("DDEGR1model.C","DDEGR1model()",CVS_verNum);
    ...
    if (psr[p].param[param_afac].paramSet[0]==1) afac = getParameterValue(&psr[p],param_afac,0);
    else afac = 0.0;    /* WHAT SHOULD THIS BE??? */
    ...
    if (psr[p].param[param_xomdot].paramSet[0]==1) xkex = psr[p].param[param_xomdot].val[0]/(rad2deg*365.25*86400.0*an);
    else xkex=0;
    ...
    double eccz=0;
    double xz=0;
    if (psr[p].param[param_zeta3].paramSet[0] == 1){
        zeta3 = psr[p].param[param_zeta3].val[0];
        eccz = sqrt(1-zeta3*zeta3); 
        xz   = sqrt(powl(zeta1*eccz,2.0)+powl(zeta2/zeta3,2.0));
        omz  = atan2(zeta1*eccz,zeta2/zeta3)*180/M_PI;
    }
    ...
    mass2ddegr(am,am2,x,ecc,an,&arr,&ar,&xk,&si,&gamma,&pbdot);
    ...
    if (psr[p].param[param_zeta3].paramSet[0] == 1) u=M_PI;
    else u=phase+ecc*sin(phase)*(1.0+ecc*cos(phase));

    do {
    	fac = 1.0/(1.0-ecc*cos(u));  /* NOTE COULD BE WRONG IN DDmodel - SEE USE OF FAC !!!! */
        du=(phase-(u-ecc*sin(u)))*fac; 
        u=u+du;
    } while (fabs(du)>1.0e-12);  /* 1e-12 in DDmodel */

    /*  DD equations 17a, 29 */
    ae = 2.0*atan(sqrt((1+ecc)/(1-ecc))*tan(0.5*u));
    if(ae<0.0) ae=ae+2.0*M_PI;
    ae = 2.0*M_PI*orbits + ae-phase;
    omega=omz/rad2deg + k*(ae-2.0*M_PI*orbits) + xkex*2.0*M_PI*orbits;
    ...
    if (brace<=0)
    {
        printf("ERROR: In DDEGR1 model, brace < 0\n");
        printf("mc: %lf mtot: %lf\n",am2,am);
        exit(1);
    }
    dlogbr=log(brace);
    ds=-2*m2*dlogbr;
...
void updateDDEGR1(pulsar *psr,double val,double err,int pos)
{
    ...
    else if (pos==param_zeta1 || pos==param_zeta2 || pos==param_zeta3 || pos==param_a1 || pos==param_ecc || pos==param_t0 || pos==param_sini || pos==param_m2
            || pos==param_mtot )
    {
        ...
        double *xk,double *si,double *gamma,double *pbdot)
{
    ...
    if (m<0)
    {
        printf("ERROR: problem in DDEGR1 model (mtot < 0)\n");
        exit(1);
    }
